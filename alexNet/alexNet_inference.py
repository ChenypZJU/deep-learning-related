...
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope('layer1-conv1'):
        ...
        conv1=tf.nn.conv2d(input_tensor,weights,strides=[1,conv1_stride,conv1_stride,1],padding='VALID')
        relu1=tf.nn.relu(tf.nn.bias_add(conv1,conv1_biases))
        
    with tf.name_scope('layer2-pool1'):
        pool1=tf.max_pool(relu1,ksize=[1,pool1_size,pool1_size,1],strides=[1,pool1_stride,pool1_stride,1],padding='VALID')
    ...
    with tf.variable_scope('layer4-conv2'):
        ...
        conv2=tf.nn.conv2d(norm1,weights,strides=[1,conv2_stride,conv2_stride,1],padding='SAME')
        relu2=tf.nn.relu(tf.nn.bias_add(conv2,conv2_biases))
    
    with tf.name_scope('layer5-pool2'):
        pool2=tf.max_pool(relu2,ksize=[1,pool2_size,pool2_size,1],strides=[1,pool2_stride,pool2_stride,1],padding='VALID')
    ...
    with tf.variable_scope('layer7-conv3'):
        weights=tf.get_variable("weight",[conv3_size,conv3_size,conv2_deep,conv3_deep],
                                initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv3_biases=tf.get_variable("bias",[conv3_deep],initializer=tf.constant_initilizer(0.0))
        
        conv3=tf.nn.conv2d(norm2,weights,strides=[1,conv3_stride,conv3_stride,1],padding='SAME')
        relu3=tf.nn.relu(tf.nn.bias_add(conv3,conv3_biases))
    
    with tf.variable_scope('layer8-conv4'):
        weights=tf.get_variable("weight",[conv4_size,conv4_size,conv3_deep,conv4_deep],
                                initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv4_biases=tf.get_variable("bias",[conv4_deep],initializer=tf.constant_initilizer(0.0))
        
        conv4=tf.nn.conv2d(relu3,weights,strides=[1,conv4_stride,conv4_stride,1],padding='SAME')
        relu4=tf.nn.relu(tf.nn.bias_add(conv4,conv4_biases))
        
    with tf.variable_scope('layer9-conv5'):
        weights=tf.get_variable("weight",[conv5_size,conv5_size,conv4_deep,conv5_deep],
                                initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv5_biases=tf.get_variable("bias",[conv5_deep],initializer=tf.constant_initilizer(0.0))
        
        conv5=tf.nn.conv2d(relu4,weights,strides=[1,conv5_stride,conv5_stride,1],padding='SAME')
        relu5=tf.nn.relu(tf.nn.bias_add(conv5,conv5_biases))
        
    with tf.name_scope('layer10-pool3'):
        pool3=tf.max_pool(relu5,ksize=[1,pool3_size,pool3_size,1],strides=[1,pool3_stride,pool3_stride,1],padding='VALID')     
    ...
    with tf.variable_scope('layer11-fc1'):
        weights=tf.get_variable("weight",[nodes,fc1_size],initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer !=None:
    ...
    with tf.variable_scope('layer12-fc2'):
        weights=tf.get_variable("weight",[fc1_size,fc2_size],initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer !=None:
            tf.add_to_collection('losses',regularizer(weights))
    ...
    with tf.variable_scope('layer13-fc3'):
        weights=tf.get_variable("weight",[fc2_size,fc3_size],initializer=tf.truncated_normal_initializer(stddev=0.1))
        if regularizer !=None:
            tf.add_to_collection('losses',regularizer(weights))
        biases=tf.get_variable('bias',[fc3_size],initializer=tf.constant_initilizer(0.0))
        
        fc3=tf.matmul(fc2,weights)+biases #不确定是否还是要relu函数
